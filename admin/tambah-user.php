...
if (isset($_POST['edit'])) {
   ...
   $newFileName = pathinfo($fileName, PATHINFO_FILENAME) . "-" . $tanggal . "-" . $detik . "-" .str_pad($id, 5, '0', STR_PAD_LEFT) . "." . $fileExtension;
   ...
   if (!empty($fileName)) {
    if (pathinfo($fileDestination, PATHINFO_EXTENSION) === 'png' || pathinfo($fileDestination, PATHINFO_EXTENSION) === 'jpg' || pathinfo($fileDestination, PATHINFO_EXTENSION) === 'jpeg') {
        ...
        if (!empty($rowEdit['foto']) && file_exists($targetDir . $rowEdit['foto'])) {
            unlink($targetDir . $rowEdit['foto']);
        }

        //Pindahkan file ke folder upload
        if (move_uploaded_file($_FILES['foto']['tmp_name'], $fileDestination)) {
            $qEdit = "UPDATE user SET nama = '$nama', email = '$email', password = '$password', foto = '$newFileName', created_at = now(), updated_at = now()  WHERE id = $id";
        }else {
            header("Location: tambah-user.php?edit=$id&status=gagal-upload");
            exit();
        }
    }else {
        header("Location: tambah-user.php?edit=$id&status=gagal-format");
        exit();
    }
   }else{
        //Jika tidak ada file upload
        $qEdit = "UPDATE user SET nama = '$nama', email = '$email', password = '$password', created_at = now(), updated_at = now()  WHERE id = $id";
   }
   if (mysqli_query($koneksi, $qEdit)) {
        header("Location: user.php?ubah=berhasil");
        exit();
   }else{
    header("Location: tambah-user.php?edit=$id&status=gagal-update");
    exit();
   }
